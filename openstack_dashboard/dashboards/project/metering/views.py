...
class SamplesView(TemplateView):
    ...
    @staticmethod
    def _series_for_meter(aggregates, resource_name, meter_name, stats_name,
                          unit):
...
class ReportView(tables.MultiTableView):
    ...
    def get_context_data(self, **kwargs):
        return {'tables': self.get_tables().values()}
...
def query_data(request, date_from, date_to, date_options, group_by, meter,
               period=None):
    ...
    if group_by == "project":
        try:
            tenants, more = api.keystone.tenant_list(request, domain=None,
                                                     paginate=False)
            queries = {}
            for tenant in tenants:
                tenant_query = [{"field": "project_id", "op": "eq",
                                 "value": tenant.id}]
                queries[tenant.name] = tenant_query

        except Exception:
            tenants = [{'id': request.user.tenant_id}, ]
            queries = {request.user.tenant_name: [{"field": "project_id",
                                                   "op": "eq",
                                                   "value": request.user.tenant_id}]}
            # exceptions.handle(request,
                              # _('Unable to retrieve the list of tenants.'))
